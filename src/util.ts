...
// ── CRC32 (polynomial 0x04C11DB7, initial 0, non-reflected) ──

/** Lookup table for CRC32, hardcoded from util.c (polynomial 0x04C11DB7, initial 0). */
 ...
 * Aligns with util.c UtilChecksumCrc32Calculate (non-reflected, no final XOR).
 */
export function utilChecksumCrc32Calculate(data: Uint8Array): number {
  let result = 0;
  for (let i = 0; i < data.length; i++) {
    result = ((result << 8) ^ CRC32_TABLE[(data[i] ^ (result >>> 24)) & 0xff]) >>> 0;
  }
  return result;
}
