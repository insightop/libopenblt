...
function extractLineData(line: string): { address: number; data: Uint8Array } | null {
  ...
  let addrLen: number
  switch (lineType) {
    case SREC_LINE_TYPE_S1: addrLen = 2; break // 16-bit address
    case SREC_LINE_TYPE_S2: addrLen = 3; break // 24-bit address
    case SREC_LINE_TYPE_S3: addrLen = 4; break // 32-bit address
    default: return null
  }
...
function srecSaveToFile(segments: FirmwareSegment[]): string {
  ...
  const maxAddr = segments.length > 0
    ...
    : 0
  const dataLineType = maxAddr > 0xFFFFFF ? SREC_LINE_TYPE_S3
    : maxAddr > 0xFFFF ? SREC_LINE_TYPE_S2
    : SREC_LINE_TYPE_S1
  const termLineType = maxAddr > 0xFFFFFF ? SREC_LINE_TYPE_S7
    : maxAddr > 0xFFFF ? SREC_LINE_TYPE_S8
    : SREC_LINE_TYPE_S9
  ...
  // Termination record
  lines.push(srecConstructLine(termLineType, segments.length > 0 ? segments[0].base : 0, new Uint8Array(0)))
