...
import { firmwareGetSegment, firmwareGetSegmentCount } from "./firmware.js";
import {
...
export class XcpLoader implements SessionProtocol {
  ...
  get connected(): boolean {
    return this.xcpConnected_;
  }
  get maxCto(): number {
    return this.xcpMaxCto_;
  }
  get maxProgCto(): number {
    return this.xcpMaxProgCto_;
  }
  get maxDto(): number {
    return this.xcpMaxDto_;
  }
  get isIntel(): boolean {
    return this.xcpSlaveIsIntel_;
  }
  ...
  init(settings: unknown): void {
    ...
    if (settings && typeof settings === "object") {
      ...
      this.xcpSettings_ = {
        ...
        seedKeyAlgorithm: s.seedKeyAlgorithm,
      };
      this.xcpBypassFirmwareStart_ = this.xcpSettings_.bypassFirmwareStart;
    }

    // Store the transport reference — it was already initialized by bltSessionInit
    // (aligns with C xcploader.c:206 which stores transport pointer, does NOT re-init)
    if (settings && typeof settings === "object" && "transport" in settings) {
      this.transport_ = (settings as { transport: XcpTransport }).transport;
    }

    // Initialize the XCP protection module (aligns with C xcploader.c L236)
    xcpProtectInit(this.xcpSettings_.seedKeyAlgorithm);
  }
   ...
   */
  private async sendCmdGetStatus(): Promise<{
    session: number;
    ...
    if (res.len === 8 && res.data[0] === XCPLOADER_CMD_PID_RES) {
      try {
        const secondRes = await this.transport_?.sendPacket(
          new Uint8Array(0),
          this.xcpSettings_.timeoutT6,
        );
        if (!secondRes) throw new Error("transport unavailable");
   ...
   */
  private async sendCmdGetSeed(
    resource: number,
    mode: number,
  ): Promise<{ seed: Uint8Array; remainingLen: number }> {
  ...
  private async sendCmdItCidGetInfo(): Promise<{ tableAddress: number; tableLen: number } | null> {
    ...
    try {
      ...
      const tableAddress = getOrderedLong(
        res.data.subarray(4) as Uint8Array,
        this.xcpSlaveIsIntel_,
      );
