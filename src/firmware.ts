...
export function firmwareRemoveData(address: number, len: number): boolean {
  ...
  for (const seg of segmentList) {
    ...
    if (seg.base < address && segEnd > addressEnd) {
      ...
      newSegments.push({ base: addressEnd, length: rightLen, data: seg.data.slice(leftLen + (addressEnd - address)) })
      continue
    }
    ...
    if (seg.base >= address && segEnd > addressEnd) {
      ...
      newSegments.push({ base: addressEnd, length: newLen, data: seg.data.slice(seg.length - newLen) })
    }
  }
