...
describe("xcpProtectComputeKeyFromSeed", () => {
  ...
  it("throws for invalid resource (default algorithm)", () => {
    xcpProtectInit();
    expect(() => xcpProtectComputeKeyFromSeed(0xff, new Uint8Array([1, 2, 3]))).toThrow(
      "No key algorithm",
    );
  });
  ...
  it("only accepts PGM resource (default algorithm)", () => {
    ...
    expect(() => xcpProtectComputeKeyFromSeed(XCPPROTECT_RESOURCE_STIM, seed)).toThrow(
      "No key algorithm",
    );
    expect(() => xcpProtectComputeKeyFromSeed(XCPPROTECT_RESOURCE_DAQ, seed)).toThrow(
      "No key algorithm",
    );
    expect(() => xcpProtectComputeKeyFromSeed(XCPPROTECT_RESOURCE_CALPAG, seed)).toThrow(
      "No key algorithm",
    );
  });
  ...
  it("delegates to injected algorithm", () => {
    const custom = {
      computeKeyFromSeed: (_r: number, _seed: Uint8Array) => new Uint8Array([0xff]),
