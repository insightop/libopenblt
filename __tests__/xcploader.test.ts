...
describe("XcpLoader", () => {
  ...
  it("stores transport reference (transport already initialized externally)", () => {
    const loader = new XcpLoader();
    const transport = createMockTransport();
    transport.init({ portName: "COM1", baudrate: 9600 });
    loader.init({ transport });
    // Loader should NOT re-init transport — it only stores the reference
